...
async function isVisibleById(id, timeoutMs) {
  try {
...
async function isVisibleByText(text, timeoutMs) {
  try {
...
async function ensureActiveCycle(client, userId, date) {
  ...
  if (insert.error) {
    throw new Error(`Failed to create active cycle for Detox flow: ${insert.error.message}`);
  }
}
...
describe('Real Backend Detox Journey', () => {
  ...
  beforeAll(async () => {
    ...
    if (signIn.error || !signIn.data.user?.id) {
      throw new Error(`Failed to sign in test user for Detox validation: ${signIn.error?.message}`);
    }
  ...
  it('logs in, writes data through UI, and verifies persistence in Supabase', async () => {
    ...
    if (!homeVisibleEarly) {
      ...
      await waitFor(element(by.id('email-input'))).toBeVisible().withTimeout(50000);
    ...
    try {
      await waitFor(element(by.text('Go to Home'))).toBeVisible().withTimeout(5000);
      ...
      try {
        await waitFor(element(by.text('Get Started'))).toBeVisible().withTimeout(2000);
        throw new Error(
    ...
    await waitFor(element(by.id('home-log-primary-button'))).toBeVisible().withTimeout(30000);
    await element(by.id('home-log-primary-button')).tap();
    await waitFor(element(by.text('Save Log'))).toBeVisible().withTimeout(15000);
    await element(by.text('Save Log')).tap();

    await waitFor(element(by.id('home-log-primary-button'))).toBeVisible().withTimeout(15000);
