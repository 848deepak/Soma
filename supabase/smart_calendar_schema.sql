...
create policy if not exists smart_events_select_own
  on public.smart_events
  for select
  using (auth.uid() = user_id);

create policy if not exists smart_events_insert_own
  on public.smart_events
  for insert
  with check (auth.uid() = user_id);

create policy if not exists smart_events_update_own
  on public.smart_events
  for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

create policy if not exists smart_events_delete_own
  on public.smart_events
  for delete
  using (auth.uid() = user_id);

-- Materialized suggestions for analytics/debugging; can be recomputed client-side.
...
create policy if not exists smart_event_suggestions_select_own
  on public.smart_event_suggestions
  for select
  using (auth.uid() = user_id);

create policy if not exists smart_event_suggestions_insert_own
  on public.smart_event_suggestions
  for insert
  with check (auth.uid() = user_id);
